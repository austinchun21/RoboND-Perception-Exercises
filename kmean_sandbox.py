...
def cluster_gen(n_clusters, pts_minmax=(10, 100), x_mult=(1, 5), y_mult=(1, 5), 
...
for k_clusters in k_clusters_range:
    ...
    print("k: %d, compactness: %.3f"%(k_clusters,compactness))
    elbow.append(compactness)
    ...
    for idx, xpts in enumerate(kmeans_clusters_x):
        ...
        plt.xlim(min_x, max_x)
        plt.ylim(min_y, max_y)
        plt.title('k-means Clusters, K=%d'%k_clusters, fontsize=20)
    fig.tight_layout()
...
fig2 = plt.figure()
plt.plot(k_clusters_range, elbow, 'o-')
plt.show()


# k_clusters = input("What K looks the best: ")
